        ...
        if($_SESSION["admin"]=='admin'){
            echo '<div style="width: 400px;" class="questionnaire">
	               <h2 class="logheader">
                    ...
                    $names = all_names_model($link);
                    echo('<table class="jointable">');
                    foreach($names as $name){
           
                        echo('<tr><td><a href="list/index.php?id='.$name["id"].'">'.$name["name"].'</a></td>');
                        echo('<td><a href="../../main.php?action=delete&name='.$name["name"].'">Удалить</a></td>');
                        echo "</tr>";
                    }
                    echo('</table>');
                    echo '	<span style="margin-top: 0px;" class="btn_container">
    
                            <div class="btn"><a href="/admin/newmodel/">Добавить модель</a></div> 
                            </span>
